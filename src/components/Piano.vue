          ...
          <div class="bkey-wrap bkey-wrap1">
            ...
            >
              <div class="keytip">
                <div
                  class="keyname"
                  v-html="[note.key, note.name].join('\n')"
                  v-show="showKeyName"
                ></div>
              </div>
            </div>
        ...
        setTimeout(() => {
          if (note.includes("]")) {
            let s = note
              ...
              .split(",");
            s.forEach(n => {
              const pressedNote = this.getNoteByNameCode(n);
              ...
              setTimeout(() => {
                ...
                $(`[data-keyCode=${pressedNote.keyCode}]`).removeClass(
                  "auto-key-active"
                );
              }, duration * 1000);
            });
            ...
            if (note) {
              const pressedNote = this.getNoteByNameCode(note);
              // console.log(pressedNote);
              $(`[data-keyCode=${pressedNote.keyCode}]`).addClass(
                "auto-key-active"
              );
